...
function ResultsPage(props){
    ...
    const [username, setUser] = useState(window.location.href.slice(45))
    const [stuff, setData] = useState({})
    const [data, setStuff] = useState([
        { title: 'Sample Topic One', value: 10, color: '#E38627' },
        { title: 'Sample Topic Two', value: 15, color: '#C13C37' },
        { title: 'Sample Topic Three', value: 20, color: '#6A2135' },
    ])
    ...
    useEffect(() => {
        ...
        const result = await axios.get('http://localhost:5000/api/analytics?Username='+username)
        setData(result.data)
        console.log(result.data)
        var toret = []
        var col = ""
        for (let i = 0; i < result.data.scores.length; i++) { 
            col = randomColor()
            toret.push({title: result.data.vectors[i], value: result.data.scores[i], color: col})
        }
        console.log(toret)
        setStuff(toret)
    }
